...
function getWeatherDetails(cityName, lat, lon) {
    ...
    fetch(WEATHER_API_URL).then(function (res) {
        ...
        alert('Sorry, it seems we are experiencing some issues looking for the weather forecast!');
    });
}
...
function getCityCoordinates() {
    ...
    fetch(GEOCODING_API_URL).then(function (res) {
        ...
        if (!data.length) return alert('Sorry! It seems ' + cityName + ' does not exist!');
